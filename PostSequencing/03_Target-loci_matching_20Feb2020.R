...
#OR on the cluster (installs the needed dependencies)
...
###########################################################################################
#####################  1. Match target loci to contigs         ############################
###########################################################################################

#This script does the following:
#1. Matches the target loci to the contigs, saves them to a new file
#2. Also finds the potential paralogs, removes them, and saves them to a separate file

#Set up directories
threads     <-"6"             # threads, keep quotes
query.type  <-"loci"          # choose loci if the provided file contains target loci rather than probes
contig.save <-"Project_Name"  # This is your save name for the big contig match file

#CLUSTER directories
work.dir    <-"/Users/Jeff/Google Drive/KU/SnakeCap"
proc.dir    <-"/Users/Jeff/Google Drive/KU/SnakeCap/SnakeCap_Data/SnakeCap1_Spades-contigs"
contig.dir  <-"/Users/Jeff/Google Drive/KU/SnakeCap/SnakeCap_Data/SnakeCap1_Spades-contigs/Assembled_Contigs"
loci.file   <-"/Users/Jeff/Downloads/Weinell_TargetLoci_Snakes_Final_18April2019.fa"
# probe.file  <- "/Users/Jeff/Downloads/Weinell_FinalProbeSet_20020Probes_7-Oct-2018.fasta"  ### probes are not needed in this step

#/Applications/ncbi-blast-2.5.0+/bin/makeblastdb
blastPath <- "/Applications/ncbi-blast-2.5.0+/bin/"

########################################################################
# STEP 1
# Matches contigs to target loci (or probes)
########################################################################

# load in target loci file

if(query.type=="probe"){
	probe.seqs <- scanFa(FaFile(probe.file))
	query.file <- probe.file
	query.seqs <- probe.seqs
	
} else {
	loci.seqs  <- scanFa(FaFile(loci.file))
	query.file <- loci.file
	query.seqs <- loci.seqs
}

#Make blast database from target loci
system(paste(blastPath,"makeblastdb -in ", loci.file, " -parse_seqids -dbtype nucl "," -out ", work.dir, "/blast_db", sep = ""))
...
out.dir.name  <- "assembled-contigs"

#Matching and processing for each sample

for (i in 1:length(file.names)){

	#Sets up working directories for each species (aka each sample)
	sample            <- gsub(pattern = ".fa$", replacement = "", x = file.names[i])
	species.dir       <- paste(proc.dir, "/", sample, sep = "")
	out.dir.temp.path <- paste(species.dir, "/", out.dir.name, sep = "")

	#Creates species directory if none exists  
	dir.check.create(species.dir)
	dir.check.create(out.dir.temp.path)
	
	## move into the output directory for the ith sample
	setwd(out.dir.temp.path)
	
	# DEDUPE almost exact duplicate removal
	system(paste("dedupe.sh in=",contig.dir, "/", file.names[i], " ordered=t overwrite=true "," out=", sample, "_dd.fa", " minidentity=97", sep = ""), ignore.stderr = T)
	
	#Matches samples to loci
	system(paste("blastn -task dc-megablast -db ", work.dir, "/blast_db"," -query ", sample, "_dd.fa -out ", sample, "_match.txt", " -outfmt 6 -num_threads ", threads, sep = ""))
	
	# Loads in the hit table
	...
	if (query.type == "probe"){
		...
		names(query.seqs) <- gsub(pattern = "_.*", replacement = "", x = names(query.seqs))
	}#end probe if
	...
	#Gets rid of very poor matches (if >40 matches, removes matches with e-value <= 0.05)
	...
	#Tests if matches are overlapping
	...
	#Gets contig lengths from contigs file
	...
	# Gets length of contigs matching region from the target/probe loci
	new.tsize <- width(query.seqs)[pmatch(filt.data$tName, names(query.seqs), duplicates.ok = T)]
	filt.data[,tSize:=as.numeric(new.tsize)]
	
	#Removes matches that barely match to the full contig (i.e., that match to a small, 20% of, region of the full contig)
	...
	##############################################################
	# Part B: Finds different contigs that match to the same locus
	##############################################################
	...
	for (j in 1:length(loci.names)){
		...
		for (k in 1:(nrow(sub.match)-1)){
			if ((sub.match$tStart[k+1]-sub.match$tEnd[k]) < (-20)){
				hit.para <- 1
			}
		}
		...
		if (hit.over == 1){
			...
			for (k in 1:length(spp.seq)){
				new.seq <- append(new.seq, subseq(x = spp.seq[k], start = sub.match$qStart[k], end = sub.match$qEnd[k]))
			}
	...
	#######################################################################
	## Part C: Finds the same contig that match to different target loci ##
	#######################################################################
	...
	#Pulls out the double-loci contigs
	...
	for (j in 1:length(dup.loci)){
		...
		for (k in 1:length(starts)){
			new.seq<-append(new.seq, subseq(x = spp.seq, start = starts[k], end = ends[k]))
		}
	...
	# DUPES and numbers dont match up between contigs and table (dupes or not removed?)
	temp <- fin.loci[duplicated(names(fin.loci)) == T]
	if(length(temp) != 0){
		stop("DUPLICATE FOUND")
	}
	
	#Finds target loci that match to two or more contigs
	final.loci <- as.list(as.character(fin.loci))
	write.fasta(sequences = final.loci, names = names(final.loci), paste(sample, "_orthologs.fa", sep = ""), nbchar = 1000000, as.string = T)
	...
	write.fasta(sequences = write.loci, names = names(write.loci), paste(sample, "_paralogs.fa", sep = ""), nbchar = 1000000, as.string = T)
	
	system(paste("rm ", sample, "_dd.fa ", sep = ""))  
	system(paste("rm ", sample, "_match.txt", sep = ""))  
	print(paste(sample, " target-loci matching complete. ", length(final.loci), " found!"))
...
for (i in 1:length(samples)){
	...
	setwd(paste(proc.dir, "/", samples[i], "/", out.dir.name, sep = ""))

	#Gets length of raw contigs with no matches
	...
	#Gets lengths of raw contigs of orthologs
	...
	#Gets length of raw contigs of paralogs
