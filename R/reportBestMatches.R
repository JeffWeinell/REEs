...
#' @param remove.subseq.matches Not yet implemented. In future: Logical with whether or not to drop matches for which the subject sequence is a subsequence of another match. Default is FALSE.
#' @param min.bitscore Number indicating the minimum bitscore required to keep the best match. Matches with a bitscore below this value are not included in the output table. Default = 0.
#' Setting min.bitscore to at least 50 may be a way to filter likely paralogs (doi: 10.1002/0471250953.bi0301s42).
...
#' @param blast.method Not yet implemented. In future: If set to "tblastn", in which case the translation frame suffix is removed from query sequence IDs (if present). In the future, I will either delete this argument or this argument will take as input one of the following: NULL (default), or one of "blastn", "blastp", "blastx", "tblastn", "tblastx".
#' @param table.format At present, only format 6 can be used. If more than 12 columns are present only the first 12 are used and assumed to correspond to the columns of format 6. In the future: Number indicating which NCBI table format is applicable to input.table.
...
reportBestMatches <- function(input.table, output.table.path=NULL, remove.subseq.matches=F, min.bitscore=0, min.bitscore.difference=0,blast.method=NULL,table.format=6){
	if(table.format!=6){
		stop("Only NCBI format 6 tables can be processed")
	} else {
	...
	# Set column names to format 6 names
	colnames(all.matches) <- c("qseqid","sseqid","pident","length","mismatch","gapopen","qstart","qend","sstart","send","evalue","bitscore")

	# filter the table to the best matches
	best.matches <- all.matches %>% 
	                dplyr::mutate(pairseqid=paste(qseqid,sseqid,sep="_")) %>%
	                dplyr::mutate(pairseqid2=paste0("qs",match(pairseqid,unique(pairseqid)))) %>%
	                dplyr::grouped_df(vars="qseqid") %>% 
	                dplyr::arrange(dplyr::desc(bitscore),.by_group = T) %>% 
	                dplyr::mutate(bitrank=rank(dplyr::desc(bitscore),ties.method="first")) %>% 
	                dplyr::mutate(bitmax = max(bitscore)) %>% 
	                dplyr::mutate(bitdiff = bitmax - bitscore) %>% 
	...
	if(!is.null(output.table.path)){
		write.table(best.matches.out,file=output.table.path,sep="\t",row.names=F,col.names=T,quote=F)
	}
	# Coerce the result (the best hits table) to a data.table object because it is safe to print.
	result <- data.table::as.data.table(best.matches.out)
	result
}
#' @examples
#' ### Return the best match to each query sequence
#' all.hits  <- fread("https://osf.io/8x2yb/download",header=T,sep="\t")
#' best.hits <- reportBestMatches(all.hits)
