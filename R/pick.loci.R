#' @title Calculate Flanking Lengths
#' 
...
#' 
#' @param locus.length Numerical vector of loci lengths. Must be length one if type ="both"
#' @param target.length Length of region that includes the locus and is covered by baits.
#' @return List with two numerical vectors holding lengths of upstream and downstream regions targetted.
...
#' @title Calculate Bait Coverage Length and Number of Baits
#' 
...
#' @title Pick Rapidly-evolving Exons
#' 
...
#'
#' @param statsTable Data frame or data table object holding the table generated by the makeStatsTable function, or, a character string with the path to this table.
#' @param reference.species The species whose contigs are in the first column of the input table of the statsTable argument. If NULL (the default), "referenceSpecies" is used. For each locus, the other species were aligned to this species when the function makeStatsTable was run.
...
#' @param min.num.species Either the minumum number of species required to have the locus, or the character string "all" (the default), which means that the locus must be present in all of the species or else it is filtered.
#' @param max.capture.coverage Maximum number of nucleotides that can be targeted. Default is NULL.
#' @param bait.tiling Fraction overlap between adjacent probes (baits); value of 0.5 corresponds to 50% overlap between adjacent baits Default is NULL. Ignored if bait.length is NULL.
#' @param bait.length Number indicating nucleotide length of baits that will be used for capturing loci. Default is NULL
#' @param bait.number Number of unique baits available for sequence capture.This is typically a constraint of the bait kit purchased. Default is NULL.
#' @param flanking.length Either NULL (default), "auto", or a number indicating the how many bases upstream and downstream of REEs should also be targeted. Default is NULL.
...
pick.loci <- function(statsTable, output.path=NULL, reference.species=NULL, species.subgroup=NULL, pident.keep=c(65,100), max.loci.per.gene=NULL, min.num.species="all", max.capture.coverage=NULL, bait.tiling=NULL, bait.length=NULL, bait.number=NULL, flanking.length=NULL, plot.results=FALSE, use.min.pident.subgroup=FALSE, fast.stat="pident"){
	...
	### If bait.number is non-NULL, and either bait.length or bait.tiling are NULL, then stop with a warning message.
	if(!is.null(bait.number) & (is.null(bait.length) | is.null(bait.tiling))){
			stop("If bait.number is not NULL, then bait.length and bait.tiling must also not be NULL")
	}
	### Stop with a warning if a file with output.path already exists
...
#		stats.data.exome.ordered      <- stats.data.exome[with(stats.data.exome, order(gene.name, mean.pident, decreasing=F)),] ### Sorts loci by gene name, and then by increasing percent identity to the reference species.
		### Sort loci by gene name and then by increasing percent identity to the reference species.
...
#		stats.data.exome.ordered      <- stats.data.exome[with(stats.data.exome, order(gene.name, percentPIS, decreasing=T)),] ### Sorts loci by gene name, and then by decreasing percent of sites parsimony informative.
	...
	if(!all((c("min.pident.all","min.pident.subgroup") %in% colnames(stats.table1)))){
		...
		if(!("min.pident.all" %in% colnames(stats.table1))){
			...
			# min.pident.all          <- round(apply(X=stats.data.exome.ordered[,..all.pident.columns],MARGIN=1,FUN=min),digits=2) #| [NOTE: Must have the ".." before the object "all.pident.columns" to specify that it is an integer index.] This line Calculates minimum percent identity (among all species to the reference species)
			# Must have the ".." before the object "all.pident.columns" to specify that it is an integer index.] This line Calculates minimum percent identity (among all species to the reference species)
		...
		if(!("min.pident.subgroup" %in% colnames(stats.table1)) & !is.null(species.subgroup)){
				if(is.integer(species.subgroup)){
					...
					# Calculates minimum percent identity (among subgroup species to the reference species)
	...
	#############
	#### Filter loci if minimum percent identity to the reference.species is < pident.keep[1] or >= pident.keep[2] (usually 65% and 100%, respectively)
...
#			stats.table3 <- stats.table2[to.keep2,]
	...
	#	}
	#}
	# Orders rows of stats table by increasing mean percent identity to the reference species
	if(fast.stat == "pident"){
	...
	###### Creates a column holding loci lengths for the reference species, if such a column doesnt already exist. This is calculated from the names used in the first column, and therefore this won't work if the first column does not include start and end positions.
	#if(length(which(colnames(stats.table4)==paste0("locus.length.",mgsub(c(" ","-"),c("_","_"),reference.species))))==0){
	if(length(grep("^locus.length",colnames(stats.table4)))==0){
		...
		# colnames(stats.table4)[which(colnames(stats.table4)=="loci.lengths")] <- paste0("locus.length.",mgsub(c(" ","-"),c("_","_"),reference.species))
		colnames(stats.table4)[which(colnames(stats.table4)=="loci.lengths")] <- "locus.length.ReferenceSpecies"
	} else {
		# loci.lengths <- stats.table4[,which(colnames(stats.table4)==paste0("locus.length.",mgsub(c(" ","-"),c("_","_"),reference.species)))]
	...
	if (plot.results==T){
		if(is.null(reference.species)){
			reference.species <- "referenceSpecies"
		}
		...
		text(x=c(((par("usr")[2]-par("usr")[1])/10)+par("usr")[1]),y= c(((par("usr")[4]-par("usr")[3])/2.5)+par("usr")[3]),labels= paste("100% > minimum similarity to ",gsub("_"," ",reference.species)," ≥ ",pident.keep[1],"%",sep=""),cex=1,col=1,adj=c(0,NA))
		...
		mtext(paste("% of sites identical to ", gsub("_"," ",reference.species)),c(SOUTH=1),line=2, cex=1, outer=TRUE)
