...
#' @param minwidth Number indicating the minimum alignment width to retain an alignment. Default 40.
...
#' @export maf2fasta
maf2fasta <- function(maf,outdir,samples=NULL,minwidth=40,minsamples=4,silent=F){
	...
	for(i in 10001:length(lines.a)){
		...
		mat.temp    <- do.call(rbind,strsplit(lines.temp,split="\t"))[,-1]
		idmat.temp  <- do.call(rbind,strsplit(mat.temp[,1],split="\\."))
		mat.temp2   <- cbind(idmat.temp,mat.temp[,-1])
		...
		if(!is.null(samples)){
			...
			if(any(mat.temp2[,"SampleName"] %in% samples)){
				mat.temp3 <- mat.temp2[which(mat.temp2[,"SampleName"] %in% samples),]
			} else {
				next
			}
		} else {
...
# maf="/panfs/pfs.local/home/j926w878/scratch/scratch_v3/SequenceCapture/SnakeCap_AllSamples/Cactus/TestSnakesSubset.maf"
# outdir  <- "/panfs/pfs.local/home/j926w878/scratch/scratch_v3/SequenceCapture/SnakeCap_AllSamples/Cactus/test"
# samples <- c("Ahaetulla","Aplopeltura","Boiga","Elapsoidea","Hemibungarus")
# minwidth=100
# minsamples=2
