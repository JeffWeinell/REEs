...
5. Align shared exons and calculate stats. I used the R function **makeExomeStatsTable** to do all of the following:
  1. obtain the set of T. sirtalis exons present in all exomes (from step 3), i.e., the shared exons
  2. perform multiple sequence alignment (MAFFT algorithm) for each of the shared exons
  3. calculate a set of stats for each shared exon alignment, and save results to the file **stats_exome_data_TBLASTX.txt** (results: includes stats for 66,489 alignments). 
